...
public struct ReorderableView<Item: Reorderable, Content: View, Preview: View>: View {
    ...
    public init(
        ...
        dropAction: @escaping (IndexSet, Int) -> Void
    ...
    private let dropAction: (IndexSet, Int) -> Void
...
struct ReorderableDragRelocateDelegate<Item: Reorderable>: DropDelegate {
    ...
    var dropAction: (IndexSet, Int) -> Void
    ...
    func performDrop(info: DropInfo) -> Bool {
        ...
        if let current = active, let from = items.firstIndex(of: current), let to = items.firstIndex(of: item) {
            dropAction(IndexSet(integer: from), to > from ? to + 1 : to)
        }
        active = nil
        return true
    }
